...
print(
    f"Loading results from {file_floral} and {file_flora} with {n_samples} samples."
)
...
def plot_field(data_flora, data_floral):
    if plot_idx["automatic"]:
        idx = find_best_idx(true=data_floral["HF_field_plot"], prediction=data_floral["HF_field_prediction_plot"])
    else:
        idx = plot_idx["idx"]
    ...
    fig, axs = plt.subplots(1, 2, figsize=(12, 5), constrained_layout=True, sharey=True, sharex=True)
    axs[0].plot(field_domain, HF_field, label="High-fidelity", color="black")
    axs[0].plot(field_domain, LF_field, label="Low-fidelity", color="gray", linestyle="dashed")

    axs[1].plot(field_domain, HF_field, label="High-fidelity", color="black")
    axs[1].plot(field_domain, mean_flora, label=r"$\mu_{FLORA}$", color="green", linestyle="--")
    axs[1].fill_between(field_domain, mean_flora - sigma_factor * std_flora, mean_flora + sigma_factor * std_flora, color="green", alpha=0.3, label="")
    # axs[1].fill_between(field_domain, mean_flora - sigma_factor * std_flora, mean_flora + sigma_factor * std_flora, color="green", alpha=0.3, label=rf"$\mu_{{FLORA}} \pm {sigma_factor} \sigma$")
    axs[1].plot(field_domain, mean_floral, label=r"$\mu_{FLORAL}$", color="red", linestyle=":")
    # axs[1].fill_between(field_domain, mean_floral - sigma_factor * std_floral, mean_floral + sigma_factor * std_floral, color="red", alpha=0.3, label=rf"$\mu_{{FLORAL}} \pm {sigma_factor} \sigma$")
    axs[1].fill_between(field_domain, mean_floral - sigma_factor * std_floral, mean_floral + sigma_factor * std_floral, color="red", alpha=0.3, label="")
