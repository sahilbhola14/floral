...
def plot_field(n_train_samples):
    ...
    if plot_error:
        rel_errors = [(HF_field - means[method]).abs() for method in plot_order]
    ...
    for ii, method in enumerate(plot_order):
        ...
        # plot mean or error
        if plot_error:
            field_to_plot = (HF_field - means[method]).abs()
        else:
    ...
    (
        cbar_mean.set_label(r"$\left|w - \hat{w}\right|$")
        if plot_error
...
def plot_pareto():
    ...
    plt.tight_layout()
    plt.savefig("darcy_pareto_comparison.png", dpi=300)
    plt.close()
